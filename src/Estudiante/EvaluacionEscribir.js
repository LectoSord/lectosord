...
class EvaluacionEscribir extends React.Component{
    constructor(props) {
        super(props);
        this.state = {  
            respuesta: {}     
    ...
    async getValueFirebase(ref){
     ...
     async setValueFirebase(e,key){
        var evaluacionEscritura=e.currentTarget.value
        var nvalue=await this.getValueFirebase('/evaluacion/' +this.props.estudiante)
        if(nvalue==null)
            nvalue={respuesta:{[key]:evaluacionEscritura}}
...
export default withStyles(styles) (EvaluacionEscribir);